...
#define _DUTY_MIN 510 // servo full clockwise position (0 degree)
...
// adjust servo position according to the USS read value
  ...
  else if(180.0 <= dist_ema && dist_ema <= 360.0){
     myservo.writeMicroseconds(_DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN) / 180.0) * (dist_ema - 180.0));
  }
  else {
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max){// return 0 when out of range.
